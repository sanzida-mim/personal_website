...
function initScrollAnimations() {
    ...
    skillBars.forEach(bar => {
        ...
        const percent = bar.parentElement.parentElement.querySelector('.skill-percent');
...
if (window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
    ...
    gsap.utils.toArray('.status-badge, .location-badge').forEach((badge, index) => {
        gsap.to(badge, {
            y: -5,
            duration: 2.5,
            delay: index * 0.5,
...
// Console Easter Egg
// ================================
console.log('%c🚀 Sanzida Akter Portfolio', 'font-size: 24px; font-weight: bold; color: #00D4FF; text-shadow: 0 0 10px #00D4FF;');
console.log('%cLooking for something? Check out the source code on GitHub!', 'font-size: 14px; color: #7C3AED;');
console.log('%chttps://github.com/sanzida-mim', 'font-size: 12px; color: #00D4FF;');
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const percentElement = entry.target.querySelector('.skill-progress-percent');
